...
1. Make the preferred presentation the public-root default while retaining the
   alternate for comparison. Closer visual acceptance remains separate.
2. Configure the exact API address and browser CORS origins or an approved edge
...
3. Replace the local bounded community-summary reader with a production aggregate
   endpoint before wider use. Do not remove its cap or sum rounded per-map values.
4. Complete second-browser, narrow layout, keyboard, zoom, clipboard and actual
   website-to-installed-desktop import checks.
5. Verify actual CSP, worker/WASM behavior, referrer policy, caching, HTTPS,
   redirects and private-route isolation. Local preview headers do not prove
   the production host's behavior.
6. Confirm About/Privacy, source/download/support links and provider logging
   disclosures against the actual deployment.
7. Review bundle performance, verify the custom domain, prepare a separately

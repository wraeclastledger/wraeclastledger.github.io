...
http.createServer(async(req,res)=>{
 ...
 if(url.pathname==='/compare'){const comparison=fs.readFileSync(new URL('./compare.html',import.meta.url),'utf8');const script=comparison.match(/<script>([\s\S]*?)<\/script>/)[1];const hash=createHash('sha256').update(script).digest('base64');res.writeHead(200,{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store','Referrer-Policy':'no-referrer','Content-Security-Policy':"default-src 'self'; script-src 'sha256-"+hash+"'; style-src 'unsafe-inline'; frame-src 'self' http://127.0.0.1:43119; object-src 'none'; base-uri 'none'"});return res.end(comparison);}
